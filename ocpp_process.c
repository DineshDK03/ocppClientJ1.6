...
//According to OCPP1.6, BootNotification is the first state which registers itself to network
OCPP_States ocppstates = BootNotification;
...
int onmessage(wsclient *c, wsclient_message *msg)
{
	...
	ocppReceiveFrame = parseOCPPFrame(msg->payload);

	if(!strcmp(ocppReceiveFrame->messageCode, (char *)ocpp_message_call_code)) {
		if(!strcmp(ocppReceiveFrame->action, "ChangeAvailability")) {
			char status[20] = "Accepted";
			fprintf(stderr, "Connector Id %d\n", cJSON_GetObjectItem(ocppReceiveFrame->jsonPacket, "ConnectorId")->valueint);
			fprintf(stderr, "Connector type %s\n", cJSON_GetObjectItem(ocppReceiveFrame->jsonPacket, "type")->valuestring);

			ocppSendFrame = formOCPPFrame(ocpp_message_response_code, ocppReceiveFrame->action,ocppReceiveFrame->messageID, ChangeAvailability_resp(status));
	...
	if(isMessageSend && !strcmp(ocppReceiveFrame->messageCode, (char *)ocpp_message_response_code))
	{
		...
		switch (ocppstates)
		{
			...
			case BootNotification:
				...
				if(!strcmp(ocppReceiveFrame->messageID, ocppSendFrame->messageID))
				{
					...
					if(!strcmp(cJSON_GetObjectItem(ocppReceiveFrame->jsonPacket, "status")->valuestring,"Accepted"))
						//If CP is accepted in the network
					{
						...
						ocppstates = Authorize;      //Changing OCPPState to heartbeat
					}
			...
			break;
			case Authorize:
				ocppstates = MeterValues;
			break;
			case MeterValues:
				ocppstates = Heartbeat;
			break;
			case Heartbeat:
				ocppstates = Heartbeat;
			break;
...
void ocpp_process_start(void)
{
        ...
        char *idTag = (char *) malloc(32 * sizeof(char));
        ...
        while(isWSEstablished)
        {
        	switch (ocppstates)
        	{
        		case Authorize:
        			if(!isMessageSend) {
        				fprintf(stderr, "Sending Authorize packet\n");
        				ocppSendFrame = formOCPPFrame(ocpp_message_call_code, ocpp_states_Buffer[ocppstates], NULL, AuthorizeRequest(idTag));
        				if(0 != sendFrameToCMS(myclient, ocppSendFrame)) {
        					fprintf(stderr, "Not able to send packet, please check websocket connection\n");
        				}
        				isMessageSend = true;
			 ...
			 */
        		case BootNotification:
        			if(!isMessageSend)       //Checking if the BootNotification packet is sent or not
        			{
        				...
        				ocppSendFrame = formOCPPFrame(ocpp_message_call_code, ocpp_states_Buffer[ocppstates], NULL, bootNotificationRequest(configJson));
        		...
        		case Heartbeat:
        			...
        			if(!isMessageSend)
        			{
        				fprintf(stderr, "Sending Heartbeat Packet\n");
					printf("enum %d\n", ocppstates);
					printf("value : %s\n", ocpp_states_Buffer[ocppstates]);
        				ocppSendFrame = formOCPPFrame(ocpp_message_call_code, ocpp_states_Buffer[ocppstates], NULL, HeartbeatRequest());
        				if(0 != sendFrameToCMS(myclient, ocppSendFrame))
        				{
        					fprintf(stderr, "Not able to send packet, please check websocket connection\n");
        				}
        		...
        		case MeterValues:
					...
					char *timestamp = malloc(TIME_STAMP_LEN);
					get_utc_time(timestamp);

        				fprintf(stderr, "Sending MeterValues Packet\n");
        				ocppSendFrame = formOCPPFrame(ocpp_message_call_code, ocpp_states_Buffer[ocppstates], NULL, MeterValuesRequest(1, 2, timestamp, sample));
        				if(0 != sendFrameToCMS(myclient, ocppSendFrame))
        				{
        					fprintf(stderr, "Not able to send packet, please check websocket connection\n");
        				}
        		...
        		case StartTransaction:
					...
					char *timestamp = malloc(TIME_STAMP_LEN);
					get_utc_time(timestamp);
					struct transaction_prop t_prop = {
						.c_id = 1,

					};

        				fprintf(stderr, "Sending StartTransaction Packet\n");
        				ocppSendFrame = formOCPPFrame(ocpp_message_call_code, ocpp_states_Buffer[ocppstates], NULL, NULL);
        				if(0 != sendFrameToCMS(myclient, ocppSendFrame))
        				{
        					fprintf(stderr, "Not able to send packet, please check websocket connection\n");
        				}
